...
import { saveJob } from "@/api/savedJobs";
...
export function JobDetailPage() {
  ...
  const displayJob =
    job || mockJobs.find((j) => j.id === Number(jobId)) || mockJobs[0];
  ...
  const handleApply = async () => {
    ...
    try {
      await applyJob(String(displayJob.id));
      ...
      toast({
        ...
        description: err?.response?.data?.message || "Unable to apply.",
        variant: "destructive",
      });
    }
  };
  const handleSaveJob = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: err?.response?.data?.message || "Could not save the job.",
        variant: "destructive"
      });
    }
  };
  ...
  <img
    src={displayJob.companyLogoUrl || "/company-placeholder.png"}
  ...
  <Building className="w-10 h-10 text-gray-400" />
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center text-muted-foreground text-base gap-y-2 gap-x-4">
                  <Link
                    href={`/companies/${displayJob.companyId}`}
                  ...
                  <span className="flex items-center">
                    <MapPin className="w-4 h-4 mr-1.5" /> {displayJob.location}
                  </span>
                  <span className="flex items-center">
                    <Clock className="w-4 h-4 mr-1.5" />{" "}
                    {timeAgo(displayJob.postedAt)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-3 w-full md:w-auto mt-4 md:mt-0">
              <Button
                variant="outline"
                size="icon"
                className="glass h-12 w-12 shrink-0"
                ...
                onClick={() => {
                  if (navigator.share) {
                    navigator.share({
                      ...
                      text: `Check out this job: ${displayJob.title} at ${displayJob.companyName}`,
              ...
              {hasApplied ? (
                <Button
                  size="lg"
                  disabled
                  className="h-12 flex-1 md:w-40 bg-emerald-500/20 text-emerald-500 border border-emerald-500/30"
                >
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 space-y-8">
          ...
          <div className="flex flex-wrap gap-4">
            <Badge
              variant="secondary"
              className="px-4 py-2 text-sm bg-secondary/40 border-white/5 font-normal"
            >
              <Briefcase className="w-4 h-4 mr-2 text-muted-foreground" />{" "}
              {displayJob.experienceLevel
                ? displayJob.experienceLevel.charAt(0).toUpperCase() +
                  displayJob.experienceLevel.slice(1)
                : "Experience not specified"}
            </Badge>
            <Badge
              variant="secondary"
              className="px-4 py-2 text-sm bg-secondary/40 border-white/5 font-normal capitalize"
            >
            ...
            >
              ...
              {formatSalary(displayJob.salaryMin, displayJob.salaryMax)}
            </Badge>
            {displayJob.isRemote && (
              <Badge
                variant="secondary"
                className="px-4 py-2 text-sm bg-emerald-500/10 text-emerald-400 border-emerald-500/20 font-normal"
              >
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-lg flex items-center gap-2">
                        Strong Match <Target className="w-4 h-4 text-primary" />
                      </h3>
                      <p className="text-sm text-muted-foreground">
                        Your profile matches 5 of 6 required skills.
                      </p>
                    </div>
                  </div>
                  <Button
                    variant="outline"
                    className="glass bg-background/50 border-primary/20 text-primary hover:bg-primary hover:text-white"
                    asChild
                  >
                    <Link href="/skill-gap">
                      View Gap Analysis
          ...
          <div className="space-y-6 prose prose-invert max-w-none prose-p:text-muted-foreground prose-li:text-muted-foreground prose-headings:text-foreground">
            <div>
              ...
              <p>
                ...
                We are looking for a passionate {displayJob.title} to join our
                team at {displayJob.companyName}. In this role, you will be
                responsible for designing and developing scalable software
                solutions that impact millions of users. You'll work closely
                with product managers, designers, and other engineers to build
                innovative features in a fast-paced, collaborative environment.
              </p>
              <p>
                The ideal candidate is a self-starter who thrives in ambiguity
                and is passionate about solving complex technical challenges. If
                you love building elegant, high-performance applications and
                want to shape the future of our platform, we'd love to hear from
                you.
              </p>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {displayJob.skills.map((skill: string) => (
                  <Badge
                    key={skill}
                    variant="outline"
                    className="px-3 py-1.5 text-sm bg-background border-white/10 font-medium"
                  >
                    {skill}
                  </Badge>
                ))}
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
            ...
            <CardContent className="pt-6 space-y-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <Link
                    href={`/companies/${displayJob.companyId}`}
                    className="text-sm text-primary hover:underline cursor-pointer block mt-1"
                  >
                    View profile
                  </Link>
                </div>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center">
                    <Building className="w-4 h-4 mr-2" /> Industry
                  </span>
                  <span className="font-medium text-right">Technology</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center">
                    <Users className="w-4 h-4 mr-2" /> Size
                  </span>
                  <span className="font-medium text-right">500-1000</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center">
                    <MapPin className="w-4 h-4 mr-2" /> HQ
                  </span>
          ...
          <Card className="glass-card bg-transparent border-none shadow-none">
            ...
            <CardContent className="p-0 space-y-3">
              {mockJobs
                .filter((job) => job.id !== displayJob.id)
                .slice(0, 3)
                .map((job) => (
                  <div
                    key={job.id}
                    className="p-4 rounded-xl glass-card border border-white/5 hover:border-primary/30 transition-colors cursor-pointer"
